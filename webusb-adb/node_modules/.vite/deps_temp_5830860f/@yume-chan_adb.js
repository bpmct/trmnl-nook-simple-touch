...
function _field(size, type, serialize, deserialize, options) {
  const field2 = {
    ...
    omitInit: options == null ? void 0 : options.omitInit
  };
    ...
    serialize(source, bufferOrContext) {
      ...
      for (const [key, field2] of fieldList) {
        if (key in temp && "init" in field2) {
          const result = (_a = field2.init) == null ? void 0 : _a.call(field2, temp[key], temp);
          temp[key] = result;
        }
      }
...
function extend(base, fields, options) {
  return struct(Object.assign({}, base.fields, fields), {
    ...
    extra: base.extra,
    postDeserialize: options == null ? void 0 : options.postDeserialize
  });
}
...
function getInt64(buffer2, offset, littleEndian) {
  return littleEndian ? BigInt(buffer2[offset]) | BigInt(buffer2[offset + 1]) << 8n | BigInt(buffer2[offset + 2]) << 16n | BigInt(buffer2[offset + 3]) << 24n | BigInt(buffer2[offset + 4]) << 32n | BigInt(buffer2[offset + 5]) << 40n | BigInt(buffer2[offset + 6]) << 48n | BigInt(buffer2[offset + 7] << 24) << 32n : BigInt(buffer2[offset] << 24) << 32n | BigInt(buffer2[offset + 1]) << 48n | BigInt(buffer2[offset + 2]) << 40n | BigInt(buffer2[offset + 3]) << 32n | BigInt(buffer2[offset + 4]) << 24n | BigInt(buffer2[offset + 5]) << 16n | BigInt(buffer2[offset + 6]) << 8n | BigInt(buffer2[offset + 7]);
}
...
function setInt64(buffer2, offset, value, littleEndian) {
  if (littleEndian) {
    buffer2[offset] = Number(value & 0xffn);
    buffer2[offset + 1] = Number(value >> 8n & 0xffn);
    buffer2[offset + 2] = Number(value >> 16n & 0xffn);
...
// node_modules/@yume-chan/no-data-view/esm/int8.js
function getInt8(buffer2, offset) {
  return buffer2[offset] << 24 >> 24;
}
...
var s8 = number(1, (value, { buffer: buffer2, index }) => {
  buffer2[index] = value;
}, function* (then, reader) {
  const data = yield* then(reader.readExactly(1));
  return getInt8(data, 0);
});
var u16 = number(2, (value, { buffer: buffer2, index, littleEndian }) => {
...
var s16 = number(2, (value, { buffer: buffer2, index, littleEndian }) => {
...
var s64 = number(8, (value, { buffer: buffer2, index, littleEndian }) => {
  setInt64(buffer2, index, value, littleEndian);
}, function* (then, reader, { littleEndian }) {
  const data = yield* then(reader.readExactly(8));
  ...
  if (!ReadableStream2.prototype[Symbol.asyncIterator] || !ReadableStream2.prototype.values) {
    ReadableStream2.prototype.values = async function* (options) {
      ...
      try {
        ...
        if (!(options == null ? void 0 : options.preventCancel)) {
          await reader.cancel();
        }
        reader.releaseLock();
      }
  ...
  constructor(source, strategy) {
    ...
    super({
      start(controller) {
        ...
        return (_a = source.start) == null ? void 0 : _a.call(source, wrappedController);
      },
      pull() {
        var _a;
        return (_a = source.pull) == null ? void 0 : _a.call(source, wrappedController);
      },
      cancel(reason) {
        var _a;
        return (_a = source.cancel) == null ? void 0 : _a.call(source, reason);
      }
    }, wrappedStrategy);
  }
};
...
var ConsumableWritableStream = class extends WritableStream {
  ...
  constructor(sink, strategy) {
    ...
    super({
      start(controller) {
        var _a;
        return (_a = sink.start) == null ? void 0 : _a.call(sink, controller);
      },
      write(chunk, controller) {
        return chunk.tryConsume((chunk2) => {
          var _a;
          return (_a = sink.write) == null ? void 0 : _a.call(sink, chunk2, controller);
        });
      },
      abort(reason) {
        var _a;
        return (_a = sink.abort) == null ? void 0 : _a.call(sink, reason);
      },
      close() {
        var _a;
        return (_a = sink.close) == null ? void 0 : _a.call(sink);
      }
    }, wrappedStrategy);
  }
};
...
var createTask = (() => {
  var _a;
  return ((_a = console == null ? void 0 : console.createTask) == null ? void 0 : _a.bind(console)) ?? (() => ({
    run(callback) {
      return callback();
    }
  }));
...
var MaybeConsumableWritableStream = class extends WritableStream {
  constructor(sink, strategy) {
    ...
    super({
      ...
      write(chunk, controller) {
        return tryConsume(chunk, (chunk2) => {
          var _a;
          return (_a = sink.write) == null ? void 0 : _a.call(sink, chunk2, controller);
        });
      },
...
var AdbShellProtocolSubprocessService = class extends AdbShellProtocolSpawner {
  ...
  async pty(options) {
    ...
    if (options == null ? void 0 : options.terminalType) {
...
var AdbPublicKeyAuthenticator = async function* (credentialStore, getNextRequest) {
  ...
  const nameBuffer = ((_a = privateKey.name) == null ? void 0 : _a.length) ? encodeUtf8(privateKey.name) : EmptyUint8Array;
...
var AdbAuthenticationProcessor = class {
  ...
  dispose() {
    var _a, _b;
    void ((_b = (_a = __privateGet(this, _iterator)) == null ? void 0 : _a.return) == null ? void 0 : _b.call(_a));
  }
};
...
var AdbDaemonSocketController = class {
    ...
    this.writable = new maybe_consumable_exports.WritableStream({
      start: (controller) => {
        ...
        controller.signal.addEventListener("abort", () => {
          var _a;
          (_a = __privateGet(this, _availableWriteBytesChanged)) == null ? void 0 : _a.reject(controller.signal.reason);
        });
      },
      write: async (data) => {
  ...
  ack(bytes) {
    ...
    (_a = __privateGet(this, _availableWriteBytesChanged)) == null ? void 0 : _a.resolve();
  }
  async close() {
    var _a;
    ...
    (_a = __privateGet(this, _availableWriteBytesChanged)) == null ? void 0 : _a.reject(new Error("Socket closed"));
    try {
...
var _connection2, _buffered2, _writer6;
var AdbServerStream = class {
  ...
  async writeString(value) {
    ...
    await __privateGet(this, _writer6).write(buffer2);
  }
  release() {
  ...
  async dispose() {
    ...
    void tryClose(__privateGet(this, _writer6));
...
var AdbServerDeviceObserverOwner = class {
  ...
  async createObserver(options) {
    ...
    const includeStates = (options == null ? void 0 : options.includeStates) ?? [
      "device",
      "unauthorized"
    ];
    const observer = {
...
var _client4, _sockets2, _closed3, _disconnected2;
var AdbServerTransport = class {
  ...
  constructor(client, serial, banner, transportId, disconnected) {
    ...
    __privateAdd(this, _closed3, new PromiseResolver());
    __privateAdd(this, _disconnected2);
    __privateSet(this, _client4, client);
    this.serial = serial;
    this.banner = banner;
    this.transportId = transportId;
    __privateSet(this, _disconnected2, Promise.race([__privateGet(this, _closed3).promise, disconnected]));
  }
  get disconnected() {
  ...
  async close() {
    ...
    __privateGet(this, _closed3).resolve();
  }
};
...
var _AdbServerClient = class _AdbServerClient {
  ...
  async createConnection(request, options) {
    ...
    try {
      await raceSignal(() => stream.readOkay(), options == null ? void 0 : options.signal);
      return stream;
    } catch (e) {
      await stream.dispose();
      throw e;
  ...
  async createTransport(device) {
    ...
    const banner = new AdbBanner(info == null ? void 0 : info.product, info == null ? void 0 : info.model, info == null ? void 0 : info.device, features);
    const waitAbortController = new AbortController();
    const disconnected = this.waitForDisconnect(transportId, {
      unref: true,
      signal: waitAbortController.signal
    });
    const transport = new AdbServerTransport(this, (info == null ? void 0 : info.serial) ?? "", banner, transportId, disconnected);
...
//# sourceMappingURL=@yume-chan_adb.js.map
